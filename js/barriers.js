...
const barrier1Width = 113 * state.coefficient;
const barrier1Height = 353 * state.coefficient;

const barrier2Width = 325 * state.coefficient;
const barrier2Height = 300 * state.coefficient;

const barrier3Width = 256 * state.coefficient;
const barrier3Height = 256 * state.coefficient;

const barrier4Width = 256 * state.coefficient;
const barrier4Height = 256 * state.coefficient;

const barrier5Width = 400 * state.coefficient;
const barrier5Height = 400 * state.coefficient;

const barrier6Width = 162 * state.coefficient;
const barrier6Height = 200 * state.coefficient;

const barrier7Width = 452 * state.coefficient;
const barrier7Height = 150 * state.coefficient;

const barrier8Width = 188 * state.coefficient;
const barrier8Height = 159 * state.coefficient;

const barrier9Width = 79 * state.coefficient;
const barrier9Height = 277 * state.coefficient;

const barrier10Width = 163 * state.coefficient;
const barrier10Height = 100 * state.coefficient;

const barrier11Width = 106 * state.coefficient;
const barrier11Height = 60 * state.coefficient;

const barrier12Width = 138 * state.coefficient;
const barrier12Height = 24 * state.coefficient;

const barrier13Width = 151 * state.coefficient;
const barrier13Height = 27 * state.coefficient;

const barrier14Width = 156 * state.coefficient;
const barrier14Height = 25 * state.coefficient;

const barrier15Width = 148 * state.coefficient;
const barrier15Height = 25 * state.coefficient;

const barrier16Width = 212 * state.coefficient;
const barrier16Height = 36 * state.coefficient;

const barrier17Width = 160 * state.coefficient;
const barrier17Height = 82 * state.coefficient;
...
const arrayMoveBarrier = [null,
    function moveBarrier1() {
        barrier1.position.x -= state.speed;
    },
    function moveBarrier2() {
        barrier2.position.x -= state.speed;
    },
    function moveBarrier3() {
        barrier3.position.x -= state.speed;
    },
    function moveBarrier4() {
        barrier4.position.x -= state.speed;
    },
    function moveBarrier5() {
        barrier5.position.x -= state.speed;
    },
    function moveBarrier6() {
        barrier6.position.x -= state.speed;
    },
    function moveBarrier7() {
        barrier7.position.x -= state.speed;
    },
    function moveBarrier8() {
        barrier8.position.x -= state.speed;
    },
    function moveBarrier9() {
        barrier9.position.x -= state.speed;
    },
    function moveBarrier10() {
        barrier10.position.x -= state.speed + 1;
    },
    function moveBarrier11() {
        barrier11.position.x -= state.speed + 1;
    },
    function moveBarrier12() {
        barrier12.position.x -= state.speed + 1;
    },
    function moveBarrier13() {
        barrier13.position.x -= state.speed + 1;
    },
    function moveBarrier14() {
        barrier14.position.x -= state.speed + 1;
    },
    function moveBarrier15() {
        barrier15.position.x -= state.speed + 1;
    },
    function moveBarrier16() {
        barrier16.position.x -= state.speed + 1;
    },
    function moveBarrier17() {
        barrier17.position.x -= state.speed + 1;
    }
];

function setOneBarrier(number) {
    if (arrayBarrierSize[number][2] === 'lower') {
        arrayAllBarriers[number].anchor = {
            x: 0,
            y: 1
        };
...
function updateScore() {
    ...
    if (state.score % 3 === 0 && state.score < 82) state.speed += 0.6;
    if (state.score === 9) state.timeout = 3000;
    if (state.score === 18) state.timeout = 2500;
    if (state.score === 24) {
        state.timeout = 2000;
        state.up = 110;
        state.down = 40;
